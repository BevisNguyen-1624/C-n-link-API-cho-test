...
const mockAPI = {
  verifyReviewer: async (reviewerId: string) => {
    ...
    const reviewer = reviewers.find(r => r.reviewerId === trimmedId);
    ...
    // Count pending ideas based on assignment and completion status
    ...
    if (assignments.length > 0) {
      // Đếm số sáng kiến được phân công cho user này và chưa hoàn thành
      pendingCount = assignedIdeas.filter(a => !completed.includes(a.ideaKey)).length;
    } else {
      // Nếu chưa có phân công nào, đếm tất cả sáng kiến chưa hoàn thành
